...
insert into particulier values(null, "Val d'oises", "Alix", "Bourdin", "5 rue de la croix", "ab@gmail.com", "0666257930", "123");

insert into commande values(null, 1, curdate(), "en cours"); 

insert into appartenir values(27, 1, 1); 1

insert into facture values(null, 27, curdate(), 75.50); 1
...
drop trigger if exists inserpart;
  ...
  before insert on entreprise
    ...
    -- drop trigger if exists addArchiCom;
    -- delimiter //
    -- create trigger addArchiCom
    -- after delete on commande
    -- for each row
    -- begin
    -- insert into archiCom values (new.ref_commande, new.code_client, new.date_cmde, new.ref_produit, new.quantite_commander, new.nom, new.prix);
    -- end //
    -- delimiter ;
# -----------------------------------------------------------------------------
#      Vue
...
insert into entreprise values (null, "Val d'oises", "AB35KESEP56.166", "Norauto", "Paris", "Norauto@gmail.com", "+336.75.51.58.32", "agrafe");

update particulier set mdp_client="1242" where code_client=123;
update entreprise set mdp_client="portable" where code_client=1354;

delete from particulier where code_client=1;
delete from entreprise where code_client=1;
...
-- create table archiCom
-- as select (p.ref_commande, p.ref_produit, p.nom, p.quantite_commander, p.prix, p.date_cmde, p.prix_total)
-- from panier p;




-- create table archivCom as select * from commande where 2 = 0;
-- create table archivPanier as select * from panier where 2 = 0;
-- create table archivAppartenir as select * from appartenir where 2 = 0;
...
-- for each row
-- begin
-- insert into archivCom values (old.ref_commande, old.code_client, old.date_cmde, old.etat);
-- end //
-- delimiter ;
...
-- for each row
-- begin
-- insert into archivAppartenir values (old.ref_commande, old.ref_produit, old.quantite_commander);
-- end //
-- delimiter ;
...
-- insert into archivCom values (old.ref_commande, old.code_client, old.date_cmde, old.etat);
-- insert into archivAppartenir values (old.ref_commande, old.ref_produit, old.quantite_commander);
-- insert into archivPanier values (old.ref_commande, old.ref_produit, old.nom, old.quantite_commander, old.prix, old.img, old.date_cmde, old.prix_total);